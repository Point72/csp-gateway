...
def on_request(value: ts[Any], future: ts[object]):  # type: ignore[no-untyped-def]
    with csp.start():
...
def on_request_dict_basket(value: Dict[Any, ts[Any]], future: ts[object]):  # type: ignore[no-untyped-def]
    with csp.start():
        csp.make_passive(value)
...
def wait_for_next(value: ts[Any], future: ts[object]):
    with csp.state():
        s_futures = []
...
def wait_for_next_dict_basket(value: Dict[Any, ts[Any]], future: ts[object]):
    with csp.state():
        s_futures = []
