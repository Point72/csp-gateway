...
# TODO Replace with version in csp (for csp >= 0.0.6)
def __get_pydantic_core_schema__(cls, source_type, handler):
    ...
    def _validate(v):
        if not isinstance(v, Edge):
            raise TypeError("value must be an instance of Edge")

        value_type = ContainerTypeNormalizer.normalize_type(v.tstype.typ)
        field_type = ContainerTypeNormalizer.normalize_type(source_type)
        if get_origin(value_type) is not get_origin(field_type):
            raise TypeError(f"value tstype {v.tstype.typ} and {field_type} do not share the same Generic origin")
        if get_args(value_type) != get_args(field_type):
            raise TypeError(f"value tstype {v.tstype.typ} and {field_type} do not share the same Generic args")
        if get_origin(value_type) is None and not issubclass(value_type, field_type):
            raise TypeError(f"value tstype {v.tstype.typ} is not a subclass of {field_type}")
        return v

    return core_schema.no_info_plain_validator_function(_validate)


TsType.__get_pydantic_core_schema__ = classmethod(__get_pydantic_core_schema__)
