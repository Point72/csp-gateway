...
from pydantic import Field, FilePath, PrivateAttr, model_validator, field_validator
...
class KafkaReadWrite(GatewayModule):
    """
    ...
    *`deserialize_to_target` takes a python object, and transforms it to the target type
    ...
    def connect(self, channels: GatewayChannels):
        ...
        meta_field_map = {"timestamp": "csp_timestamp"} if self.subscribe_with_csp_engine_timestamp else None
...
class KafkaEngineReplay(EngineReplay):
    key: str = Field(
        description="""
...
# DEPRECATE. For backwards compatibility.
PubSubKafka = KafkaEngineReplay
