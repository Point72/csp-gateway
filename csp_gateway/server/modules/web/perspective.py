...
class MountPerspectiveTables(GatewayModule):
    ...
    server_views: Optional[Dict[str, ViewConfig]] = Field(default_factory=dict, description="Optional dict mapping new table name to a dict with table and view information")
    ...
    default_index: Optional[Union[str, List[str]]] = Field(None, description="Default index field for all tables, i.e. 'id'. If a multi-index is provided, will create a new computed index field.")
    ...
    def _connect_all_tables(self, channels: GatewayChannels) -> None:
        ...
        for new_table_name, view_config in self.server_views.items():
            ...
            self._table_insts[new_table_name] = self._client.table(view)
    ...
    def push_to_perspective(  # type: ignore[no-untyped-def]
        ...
        if csp.ticked(timeseries):
            ...
            # Add computed index if specified
            if isinstance(self.indexes.get(table_name, self.default_index), list):
                index_fields = self.indexes.get(table_name, self.default_index)
                index_name = "index" if "index" not in self._schema_insts[table_name] else "-".join(index_fields)
                for ts_item in timeseries:
                    computed_index = "-".join(str(getattr(ts_item, field)) for field in index_fields)
                    setattr(ts_item, "index", computed_index)
