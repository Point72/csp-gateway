...
from typing import Any, Callable, Dict, List, Literal, Optional, Union, cast, TYPE_CHECKING
...
class BaseGatewayClient(ABC):
    ...
    def _buildroute(self, route: str) -> str:
        ...
        if self._config.authenticate:
            base_url += f"?token={self._config.api_key}"
        return base_url
    ...
    def _get(
        ...
        return self._handle_response(GET(self._buildroute(route), params=params, timeout=timeout), route=route)
    ...
    async def _getasync(
        ...
        async with AsyncClient() as client:
            return self._handle_response(await client.get(self._buildroute(route), params=params, timeout=timeout), route=route)
    ...
    def _post(
        ...
        return self._handle_response(POST(self._buildroute(route), params=params, json=data), route=route)
    ...
    async def _postasync(
        ...
        async with AsyncClient() as client:
            return self._handle_response(await client.post(self._buildroute(route), params=params, json=data, timeout=timeout), route=route)
