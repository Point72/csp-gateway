...
def test_start_with_endtime(free_port):
    ...
    res = gateway.start(user_graph=user_graph, realtime=True, rest=True, endtime=timedelta(seconds=1))
...
def test_start_and_then_graph_start_error(caplog, free_port):
    ...
    try:
        gateway.start(realtime=True, rest=True, block=False, build_timeout=timedelta(seconds=10))
    except RuntimeError:
...
def test_signal_with_shutdown(signal_val, free_port):
    ...
    url = f"http://{socket.gethostname()}:{port_str}/api/v1/state"
