...
class MyReadWriteJSON(JSONEngineReplay):
...
def test_json_module_output(tmpdir, by_key):
    ...
    json_module_read = JSONEngineReplay(
        selection=channels,
        filename=file,
...
def test_custom_json_module_output(tmpdir, by_key):
    ...
    # we can make our own custom JSONEngineReplay class that specifies dynamic keys
    json_module_read = MyReadWriteJSON(
        selection=channels,
        filename=file,
        read_write_mode=ReadWriteMode.READ,
        start_writing=datetime(2099, 1, 1),  # far in the future to never stop reading
...
def test_switch_automatically_to_write(writing_start_type, tmpdir):
    ...
    json_module_read_and_write = JSONEngineReplay(selection=channels, filename=file, start_writing=start_writing)
    setter = MySetModule(
...
def test_overwrite_json(read_write_mode, tmpdir):
    ...
    setup_json_module = JSONEngineReplay(
        selection=channels,
        filename=file,
        read_write_mode=ReadWriteMode.WRITE,
    )
