...
@csp.node
def _curve(typ: "T", data: List[Tuple[Union[datetime, timedelta], "~T"]]) -> ts["T"]:
...
class GatewayTestHarness(GatewayModule):
    ...
    def connect(self, channels: GatewayChannels) -> None:
            ...
            if get_origin(outer_type) is dict:
                ...
                for k, v in basket.items():
                    ...
                    if basket_curve_data[k]:
                        curves[k] = _curve(typ, basket_curve_data[k])
                    else:
                ...
                typ = outer_type.typ
                if curve_data[channel]:
                    curve = _curve(typ, curve_data[channel])
                else:
        ...
        if event_curve_data:
            event_curve = _curve(BaseGatewayTestEvent, event_curve_data)
        else:
